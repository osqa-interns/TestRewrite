...
class TestWorkAHomework(unittest.TestCase):
    ...
    def test_working_hw(self):
        ...
        Login with the account [ student01 | staxly16 ]
        If the user has more than one course, select a Tutor course

        Click on a homework assignment on the list dashboard
        ***The user is presented with the first question of the homework assignment (t1.71.01)***

        Hover the cursor over the information icon in right corner of the footer
        ***The user is presented with the assignment description/instructions (t1.71.02)***

        [Navigate through the questions sequentially using the breadcrumbs until a free response question is found]
        Enter a free response into the free response text box
        ***The Answer button is activated
        (t1.71.04)***
        ***The user is presented with the next question when clicking the breadcrumb(t1.71.03)***
        ...
        ***A free response answer is submitted and multiple choice is shown (t1.71.05)***
        ...
        ***The Submit button is activated
        ...
        Click back to the original assessment
        ***The multiple choice answer on the original assessment is saved (t1.71.09)***
        ...
        ***The user returns to dashboard and the assignment progress shows the number of questions answered (t1.71.10)***


        Click on the same homework assignment
        Change a multiple choice answer on an assessment
        ***The button in the lower left corner says "Saving" with a loading circle
        A multiple choice answer is changed on an assessment (t1.71.12)***


        Continue answering all assessments
        ***The user is presented with the completion report at the end of the assignment (t1.71.11,14)***
        ***A homework may have a Spaced Practice assessment toward the end (t1.71.16)***
        ***A homework may have a Personalized assessment toward the end (t1.71.17)***

        Click "Back To Dashboard"
        ***The user is returned to the dashboard (t1.71.13)***
        ***The user is returned to the dashboard and the completed homework shows X/X answered in the dashboard progress column (t1.71.15)***
        ...
        :return:
        '''
        # t1.71.01 --> The user is presented with the first question of the homework assignment
        self.teacher.login()

        target_course = self.teacher.find(By.XPATH,"//div[contains(@data-title,'College Physics with Courseware')"
                                   "and not(contains(@data-title,'Debshila'))]")
        course_number = target_course.get_attribute('data-course-id') # for reference later as student
        target_course.click()

        # IMPORTANT -- get the course number of the course that you're in -->
        # this will let you identify the course later (in case there's any similarly-named courses)
        today = datetime.date.today()
        begin = (today + datetime.timedelta(days=0)).strftime('%m/%d/%Y')
        end = (today + datetime.timedelta(days=3)).strftime('%m/%d/%Y')
        # non-immediate feedback

        assignment_name = 'hw no.{0}: {1}'.format(str(randint(0,100)),str(today))
        ...
        # NOTE: exercises hard-coded to chapter 1.1 only --> might cause problems
        self.wait.until(expect.visibility_of_element_located((
        ...
        self.wait.until(
            expect.element_to_be_clickable(
                (By.XPATH, "//div[contains(@data-course-id,%s)]" % course_number))
            ).click()
        ...
        homework = self.wait.until(expect.presence_of_element_located((By.XPATH,
        ...
        # t1.71.02 --> The user is presented with the assignment description/instructions
        self.student.driver.execute_script('window.scrollBy(0, -80);')
        ...
        # t1.71.03 --> The user is presented with the next question when clicking the breadcrumb
        self.wait.until(
            expect.visibility_of_element_located(
        ...
        while(True):
            try:
                ...
                if answer_btn.get_attribute("disabled") == None:
                    break # if the answer button is activated, then you can move on
            except:
                count += 1
        ...
        # because of a bug that doesn't allow saving when navigating to review questions
        ###
        print('count',count)
        ...
        # t1.71.05 --> A free response answer is submitted and multiple choice is shown
        self.wait.until(
                    expect.element_to_be_clickable(
                        (By.XPATH, '//button[contains(@class,"btn-primary")]'))
        ).click()
        ...
        ### WHAT DO WE DO IF WE DON'T GET TO A FREE CHOICE QUESTION UNTIL WE'RE AT THE LAST QUESTION?
        ...
        # t1.71.09 --> The multiple choice answer on the original assessment is saved
        # check that multiple choice answer saved
        sections[count+increment].click()
        sections[count].click()
        mc_choices = self.wait.until(expect.presence_of_all_elements_located(
            (By.XPATH,'//div[contains(@class,"answers-answer")]'))
        ...
        # next element will be two more than the offset index of the current problem


        # t1.71.10 --> The user returns to dashboard and the assignment progress shows the number of questions answered
        ...
        # curr_question_num = int(self.student.find(By.XPATH,"//div[contains(@class,'openstax-question')]"
        #                                   ).get_attribute("data-question-number")
        #                         )
        # assert(curr_question_num == stop_point),Not
        ### NOT SURE IF WE NEED THIS ASSERT POINT HERE --> asserts that current question
        # number is at the stop point
        ...
        # T1.71.11 --> The user is presented with the completion report at the end of the assignment
        ...
        partial_complete = self.wait.until(expect.presence_of_element_located(
            (By.XPATH, locator)))
        assert("%s/%s" % (str(stop_point),str(len(sections)-1)) in partial_complete.text),\
        ...
        for q in range(len(sections)):
            try:
                ...
                pass
            if 'end' in sections[q].find_element(By.XPATH,'//i'
                ).get_attribute('class'):
                pass
            # answer the multiple choice  portion
            self.wait.until(
        ...
        # t1.71.12 --> The button in the lower left corner says "Saving" with a loading circle and
                    # A multiple choice answer is changed on an assessment (t1.71.12)***
        ...
        # t1.71.14 -->The user is presented with the completion report at the end of the assignment (same as t1.71.11)

        # Completed homework shows "You are done" in the completion report
        self.student.find(By.XPATH,"//div[@class='completed-message']")


        # t1.71.17 -->A homework may have a Personalized assessment toward the end

            ### FOR PERSONALIZED LOOK UNDER PERFORMANCE FORECAST --> WEAKEST SUBJECTS

        # T1.71.16 --> A homework may have a Spaced Practice assessment toward the end

            ### ACCOUNTS NEED MORE HISTORY FOR THESE --> MAYBE WORK THROUGH A COUPLE ASSIGNMNETS FOR THESE
            ### HOW CAN YOU MAKE SURE THAT THIS STARTS WHERE YOU LEFT OFF FROM AND ANSWERS FULLY THROUGH?
        ...
        # t1.71.15 --> The user is returned to the dashboard and the completed homework
