"""TEST COMPLETE READING ASSIGNMENT STUDENT"""
...
### NOT CERTAIN IF FINISH_READING_ASSIGNMENT WORKS
    def finish_reading_assignment(self,assignment_identifier):
        ...
        can use to find the assingment of interest on the student course dashboard
        """
        while (True):
    ...
    def test_complete_reading_assignment_student(self):
        ...
        ***The reading is marked "Complete" in the dashboard progress column (t1.28.20)***
        ...
        reading = self.wait.until(expect.presence_of_element_located(
            (By.XPATH,
             "//a[contains(@aria-labelledby,'%s') and contains(@class,'reading')]" % assignment_identifier))
        )
        ...
        reading_calendar = self.student.find(By.XPATH,
                                             "//button[contains(@class,'fa-calendar-o')]")
        ActionChains(self.student.driver).move_to_element(reading_calendar).perform()


        due_date_tooltip = self.student.driver.find_element(By.XPATH,
                                                            "//*[contains(@id,'tooltip')]")
        ...
        ### NOT SURE WHETHER I WANT TO ASSUME THAT WE BEGIN ON THE FIRST PAGE


        while(True):
            try:
                question = self.student.find(
                    By.XPATH, "//textarea[contains(@aria-labelledby,'text box')]")
        ...
        mc_choice.click()
        submit_btn = self.wait.until(expect.element_to_be_clickable((By.XPATH,
        ...
        milestone_toggle = self.student.find(By.XPATH,
        ...
        completed_message = self.student.find(By.XPATH,"//div[contains(@class,'completed-message')]")
        completed_message.find_element(By.XPATH,"//h1")
        assert('You are done' in completed_message.text),"Completion report doesn't say 'You are done'"

        # t1.28.18 --> The user views the completion
        # report and return to the dashboard
        dashboard_button = self.student.find(By.XPATH,"//a[contains(@class,'btn-primary')]")
        ...
        reading.find_element(
            By.XPATH,"//span[contains(text(),'Complete')]"
        )
