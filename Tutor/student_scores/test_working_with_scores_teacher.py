...
-Run and see where it fails -- update and add code
 ...
 ### SUPPOSED TO CHECK FOR A PROGRESS BAR... IDK HOW TO DO THIS
        # I'm having trouble finding elements in the student scores table --> it would make it easier if each
        # column heading were the parent of the cells underneath it, but I don't believe it's so
        # there's no obvious relationship in the DOM between a header and its cells?
        # this makes things like seeing if the sort by progress or sort by score functions actually work on the
...
class TestScoresReportingTeacher(unittest.TestCase):
    ...
    def test_score_stuff(self):
        ...
        overall_average1 = self.teacher.wait.until(expect.presence_of_element_located(
        ...
        overall_average2 = self.teacher.wait.until(expect.presence_of_element_located(
            (By.XPATH, "//div[contains(@class,'overall-average')]"
                                                      "//span"))
        ).text
