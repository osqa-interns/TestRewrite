"""Tutor Rewrite, Work a Late Homework."""
...
class TestWorkAHomework(unittest.TestCase):
    ...
    def teacher_make_late_assignment(self, assignment_type="homework", course_title=None, course_no="394",
        ...
        if course_title == None:  # if no specific course has been specified
            open_course_xpath = "//div[contains(@data-course-id,%s)]" % course_no

        else:
            current_term = current_tutor_term()
            course_title = course_title.lower()
            open_course_xpath = '//div[contains(@data-appearance,"{0}") and contains(@data-term,"{1}")]//a'.format(
                course_title, current_term)
        ...
        if assignment_name == None:
            assignment_name = '{0} no.{1}: {2}'.format(heading, str(randint(0, 100)), str(today_date))
        ...
        ### PROBLEM --> for some reason, the begin tuples are correct and give today's date
    ...
    def test_late_homework(self):
        ...
        *** Correctness for a completed assessment is displayed in the breadcrumbs (t1.71.21)***
        ...
        self.teacher_make_late_assignment(assignment_type='homework',course_no=course_number)
        ...
        try:
            late_icon = self.student.find(By.XPATH,"//i[contains(@class,'info late')]")
        ...
        # t1.71.20 --> A multiple choice answer is submitted and answer feedback is presented
        try:
            # if the question is two part must answer free response to get mc
            text_box = self.student.find(
                By.TAG_NAME, 'textarea')
            self.teacher.driver.execute_script("arguments[0].scrollIntoView(true);", text_box)
        ...
        try:
            ...
            correct_answer = self.student.find(By.XPATH,
                              "//div[contains(@class,'disabled answer-correct')]")


        # t1.71.21 --> Correctness for a completed assessment is displayed in the breadcrumbs
