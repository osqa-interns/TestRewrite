    ...
    def test_custom_url_student(self):
        ...
        ***Student ID changed (t2.09.33)***
        ...
        self.teacher.find(By.XPATH,
            "//p[contains(text(),'Tutor')]").click()

        # get name of course for later reference by the student
        enrollment_course_name = self.teacher.find(By.XPATH,"//a[contains(@class,'course-name')]").text
        ...
        try:
            get_started = self.student.find(By.XPATH,"//a[contains(text(),'Get started')]")
            get_started.click()

            # student id "Add Later"
            add_later = self.student.find(By.XPATH,"//button[contains(text(),'later')]")
            add_later.click()
        except:
            pass
