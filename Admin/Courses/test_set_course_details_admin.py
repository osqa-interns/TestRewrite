...
Maybe with other admin scripts? They all start out at the admin control console, so we could make
...
class TestManageDistricsSchoolsAndCourses(unittest.TestCase):
    ...
    def test_set_course_details_admin(self):
        ...
        ***End and start dates should be displayed (t2.07.11)***
        ...
        # Create the course
        create_new_course = self.admin.find(By.XPATH, "//a[text()='Add Course']")
        ...
        month_plus_one = str(right_now.month) # months are 1 minus their actual number
        print(right_now)
        print(month_now)
        print(hour_plus_one)
        ...
        self.wait.until(expect.visibility_of_element_located(
            (By.XPATH,
            '//div[contains(@class,"timepicker")]//div[@data-hour="{0}"]'.format(hour_plus_one)))).click()
        ...
        self.admin.find_all(
            By.XPATH,
            '//div[contains(@class,"timepicker")]//div[@data-hour="{0}"]'.format(hour_plus_one)
        )[-1].click()
